...
<<load_cvrisk, eval = FALSE, echo = TRUE, results = hide>>=
...
\begin{figure}[h!]
  \centering
<<crossvalidation, eval = FALSE, fig = true, results = hide, echo = false>>=
...
<<check_initial_assignement, eval = FALSE, echo = FALSE, results = hide>>=
...
<<subset, eval = FALSE, results = hide>>=
mstop(mod) <- mstop(cvr)
@
